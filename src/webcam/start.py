...
def parse_args():
    ...
    # parser.add_argument('--pretrained', type=str, default='../models/epoch17_acc0.625_compound2.pth',
    #                     help='Pretrained weights')
    # parser.add_argument('--pretrained', type=str, default='../models/epoch28_acc0.8625.pth',
    #                     help='Pretrained weights')
...
def run_test():
    ...
    test_path = '../datasets/raf-basic/EmoLabel/list_patition_label.txt'
    out_path = '../datasets/raf-basic/EmoLabel/test_out_basic.txt'

    csv_file = open(test_path, 'r')
    ...
    with torch.no_grad():
        ...
        for batch_i, (imgs, targets, _) in enumerate(val_loader):
            ...
            for i, (elem) in enumerate(outputs):
                _, predicts = torch.max(elem, 1)

                header = next(spamreader)
                out_file.write("%s %s %s\n" % (header[0], header[1],  predicts.item()+1))
                counter += 1
            loss = criterion(outputs, targets)
